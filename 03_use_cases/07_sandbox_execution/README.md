...
This shows how `muse-spark-1.3` can **execute code safely inside a throwaway Docker container**. You hand it a bug to fix; it goes into the container, reproduces the bug, fixes it, and confirms the fix by running the project's tests — all on its own — then the container is deleted. Everything it does streams to your terminal, and nothing it runs ever touches your machine.
...
- **Python 3.11+**
- A **muse-spark-1.3 API key**

```bash
export MODEL_API_KEY="<your muse-spark-1.3 key>"
...
Everything *around* the model is scaffolding that the script [`sandbox_scaffold.py`](sandbox_scaffold.py) provides — muse-spark-1.3 does not set any of this up itself:
...
## What muse-spark-1.3 is doing

**muse-spark-1.3 supplies the reasoning.** Given that scaffolding, *it* decides:
...
To point muse-spark-1.3 at a **different** bug, edit `task.json`. These are the fields that define what to fix:
...
That's the one command. Now watch the terminal — everything below happens automatically, and each screenshot is exactly what scrolls past: muse-spark-1.3's own words, the commands it runs in the sandbox, and the output it gets back.
...
![Terminal: muse-spark-1.3 runs the failing tests; the red pytest output shows an AssertionError where user:pass@ was stripped from the URL, ending in "1 failed"](./assets/02-reproduce-bug.png)
...
![Terminal: after the fix is written, muse-spark-1.3 re-runs the tests and all 5 affected cases report green PASSED, including the ones that keep user:pass@ credentials](./assets/03-fix-verified.png)

### 4. Muse Spark reports back

When it's done, muse-spark-1.3 tells you — in plain English — what was wrong and how it fixed it:

![Terminal: muse-spark-1.3's plain-English summary explaining the parse_url vs urlparse regression that dropped credentials, and that the fix rebuilds the netloc from auth, host, and port](./assets/04-summary.png)

### 5. The script verifies independently

muse-spark-1.3 already confirmed its own fix in step 3 — but you shouldn't have to take its word for it. After the model stops, the **script** re-runs the bug's two tests one more time and saves the exact code change to `pred.patch` on your machine. This whole run took 129 seconds.
...
The container is destroyed. muse-spark-1.3 ran shell commands for a couple of minutes, and none of it survives — the only thing left on your machine is `pred.patch`, the fix it wrote.
...
muse-spark-1.3 and the script both checked the fix, but the definitive test is SWE-bench's official grader. It starts a **fresh** container, applies `pred.patch`, and runs the full test suite:
