...
Working recipes for building agents and coding tools on the Meta Model API. Meta Model API is
OpenAI-compatible: keep the OpenAI client you already use and point it at one new base URL. Each
recipe is a self-contained, copy-paste starting point that proves a capability and gives you
something to build on. The default model is **Muse Spark** (`muse-spark-1.1`), which has a
1,048,576-token context window; the preview is free.

## Getting started

You need a [Model API account](https://ai.developer.meta.com/) and an API key. Store the key as
